...
Uses injected fake clients so the batch-splitting, results TXT, and the
...
def test_request_splits_into_batches_and_writes_txt(tmp_path: Path) -> None:
    ...
    txt_path = tmp_path / "GACOS" / "gacos_request_results.txt"
...
def test_download_fetches_and_imports(tmp_path: Path) -> None:
    ...
    # A staging archive is kept alongside, and a fetch results TXT is written.
    assert (tmp_path / "demo" / "GACOS" / "downloads").exists()
    assert (output_dir / "gacos_download_results.txt").exists()
