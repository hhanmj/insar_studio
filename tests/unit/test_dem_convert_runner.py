"""Tests for the DEM conversion runner + results TXT (Task 053). Offline only."""
...
def test_run_dem_conversion_writes_results_txt(tmp_path: Path) -> None:
    ...
    results_txt = tmp_path / DEM_CONVERT_SUBDIR / "dem_convert_results.txt"
    assert results_txt == summary.results_path
    rows = list(csv.DictReader(results_txt.open(encoding="utf-8"), delimiter="\t"))
