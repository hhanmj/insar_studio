...
const WORKBENCH_TOUR_VERSION = 1;
const WORKBENCH_TOUR_STEPS: TourStep[] = [
  {
    title: "欢迎使用 InSAR Assistant",
    body: "这一版把界面改成地图优先的工作台：左侧配置资源和任务，右侧一直保留地图与影像范围。下面用一分钟带你走完整个流程。",
    hint: "新手建议先按 Sentinel-1 的顺序走一遍：设置账号，选择范围，检索 ASF，核查影像，确认输出，进入下载中心。",
    placement: "center",
  },
  {
  ...
  },
  {
    ...
    title: "3. 填写参数并检索",
    body: "资源下载面板里完成 ASF 条件、SAR 文件导入、DEM 下载转换、Orbit/GACOS 日期解析等操作。每个功能都可以收起，避免信息堆在一起。",
    placement: "right",
  },
  {
  ...
  },
  {
    ...
    title: "随时重新打开引导",
    body: "右上角问号按钮可以重新播放这个引导。后续还可以继续扩展 Sentinel-2、DEM、Orbit 的专项步骤。",
    placement: "bottom",
  },
];
...
function renderOutputParameters(desc = "任务开始前确认输出根目录；留空时使用当前项目或研究区目录。") {
  ...
  function renderBeginnerGuide() {
    const guideSteps: {
      ...
      {
        title: "配置账号或密钥",
        detail: "先保存 Earthdata/ASF Token；DEM 和 GACOS 可按需要再填。",
        done: isConfigured(creds?.earthdata),
        actionLabel: "去设置",
        action: () => setPanel("settings"),
      },
      {
        title: "选择或绑定区域",
        detail: "可加载行政区、上传边界，也可以暂时不设 AOI 直接检索。",
        done: Boolean(ctx?.region?.has_aoi || ctx?.region?.bbox || focusBbox),
        actionLabel: "选区域",
        action: () => {
          setPanel("resources");
          setAoiToolsOpen(true);
        },
      },
      {
        title: "检索或导入 SAR 影像",
        detail: "推荐用 ASF 检索，也支持官方 py、metadata、metalink、目录识别。",
        done: scenes.length > 0,
        actionLabel: "Sentinel-1",
        action: () => {
          setPanel("resources");
          setSource("sentinel1");
        },
      ...
      {
        title: "核对元数据和影像框",
        detail: "确认升降轨、Path、Frame、极化、影像范围都正常显示。",
        done: Boolean(checkReport || scenes.some((scene) => scene.footprint_bbox)),
        actionLabel: scenes.length ? "核查" : undefined,
        action: scenes.length ? () => void onRunCheck() : undefined,
      },
      {
        title: "确认输出目录与并发",
        detail: "下载开始前再确认目录；并发建议从 1-3 开始。",
        done: Boolean(outputDir.trim() || asfPlan),
        actionLabel: "输出参数",
        action: () => {
          setPanel("resources");
          setSource("sentinel1");
      ...
      {
        title: "进入下载中心看队列",
        detail: "进行中、暂停、继续、结束和历史记录集中在这里。",
        done: Boolean(dlStatus || orbitStatus || downloadArchive.length),
        actionLabel: "下载中心",
        action: () => setPanel("downloads"),
      },
    ];
    ...
    return (
      <Section
        title="新手步骤指导"
        desc="按 GeoDownloader 式的流程组织：先选范围和资源，再进队列看任务。"
        icon={ListChecks}
        defaultOpen={!scenes.length}
      >
        <div className="space-y-3">
          <div className="rounded-2xl border border-white/55 bg-white/42 p-3 text-xs shadow-sm backdrop-blur-2xl dark:border-white/10 dark:bg-white/10">
            <div className="flex items-start justify-between gap-3">
              <div className="flex min-w-0 gap-2">
                ...
                <div className="min-w-0">
                  <div className="font-semibold">一分钟熟悉主界面</div>
                  <div className="mt-1 leading-5 text-muted-foreground">
                    会依次高亮顶部资源、区域范围、参数面板、地图、下载中心和设置。
                  </div>
                </div>
              </div>
              <Button type="button" size="sm" variant="outline" onClick={startWorkbenchTour}>
          ...
          <div className="space-y-2">
            {guideSteps.map((step, index) => (
              <div
                key={step.title}
                className={cn(
                  "flex items-start gap-2 rounded-2xl border px-3 py-2.5 text-xs shadow-sm backdrop-blur-2xl",
                  step.done
                    ? "border-success/25 bg-success/10"
                    : "border-white/48 bg-white/35 dark:border-white/10 dark:bg-white/8",
                )}
              >
                <div
                  className={cn(
                    "mt-0.5 flex h-6 w-6 shrink-0 items-center justify-center rounded-full text-[11px] font-semibold",
                    step.done ? "bg-success text-white" : "bg-white/70 text-muted-foreground dark:bg-white/10",
                  )}
                >
                ...
                <div className="min-w-0 flex-1">
                  ...
                  <div className="mt-0.5 leading-5 text-muted-foreground">{step.detail}</div>
                </div>
                {step.actionLabel && step.action && (
                  <Button type="button" size="sm" variant="ghost" className="shrink-0" onClick={step.action}>
                    {step.actionLabel}
                  </Button>
                )}
              </div>
  ...
  return (
    <div className="ios-window flex h-screen w-screen flex-col overflow-hidden text-foreground">
      <header className="ios-topbar z-[520] flex h-16 shrink-0 items-center gap-2 px-3" data-tour="app-header">
        ...
        <nav className="flex h-11 min-w-0 flex-1 items-center justify-start overflow-x-auto">
          <div
            className="flex items-center gap-1 rounded-[22px] border border-white/60 bg-white/42 p-1 shadow-sm backdrop-blur-2xl dark:border-white/10 dark:bg-white/10"
      ...
      <div className="flex min-h-0 flex-1">
        <aside className="ios-sidebar flex h-full w-[430px] shrink-0 flex-col">
          <div
            className="grid grid-cols-3 border-b border-white/50 bg-white/36 backdrop-blur-2xl dark:border-white/10 dark:bg-white/5"
