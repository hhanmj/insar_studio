...
function normaliseArchiveForStartup(items: DownloadArchiveItem[]) {
  return items.map(normaliseArchiveItemForStartup);
}
...
export function Workbench({
  ...
  useEffect(() => {
    ...
    if (typeof window !== "undefined") {
      window.localStorage.setItem(DOWNLOAD_ARCHIVE_KEY, JSON.stringify(downloadArchive.slice(0, 40)));
    }
    if (hasBridge() && archiveLoadedFromBridge.current) {
      void saveDownloadArchive(downloadArchive.slice(0, 40));
    }
  ...
  function rememberTask(item: { id: string; name: string; status: string; detail: string; logs?: string[] }) {
    setDownloadArchive((prev) => {
      const previous = prev.find((task) => task.id === item.id);
      ...
      return [{ ...item, logs, ts: keepPausedTime ? previous.ts : Date.now() }, ...next].slice(0, 40);
    });
  }
      ...
      <header className="ios-topbar z-[520] flex h-16 shrink-0 items-center gap-2 px-3" data-tour="app-header">
        ...
        <div className="shrink-0">
          <button
            ...
            className="flex h-10 w-[230px] max-w-[24vw] items-center justify-between gap-2 overflow-hidden rounded-2xl border border-primary/20 bg-primary/10 px-3 text-left text-sm text-primary shadow-sm backdrop-blur-xl transition-colors hover:bg-primary/15"
            data-tour="help-button"
          >
            <span className="flex min-w-0 items-center gap-2">
              <Sparkles className="h-4 w-4 shrink-0" />
              <span className="min-w-0">
                <span className="block truncate font-medium">新手引导</span>
                <span className="block truncate text-[11px] text-primary/75">
                  {ctx?.region?.has_aoi ? "AOI 已绑定" : "先选范围"} · {scenes.length} 景
                </span>
              </span>
            </span>
        ...
        <nav className="scrollbar-none flex h-11 min-w-0 flex-1 items-center justify-start overflow-x-auto">
          <div
