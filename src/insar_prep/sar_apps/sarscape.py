...
def ensure_sarscape_dem_name(region_safe_name: str, suffix: str = "") -> str:
    """Return the SARscape-ready DEM filename for a region.

    ``shiliushubao`` -> ``shiliushubao_dem`` and ``guangdong_2024`` ->
    ``guangdong_2024_dem``. The input must already be a SARscape-safe name.
    The default extensionless name is intended for ENVI rasters with a sidecar
    ``.hdr``. A dotted suffix may still be passed for legacy callers.
    """
    ...
    if "_ellipsoid" in name:
        msg = "SARscape-ready DEM must not be an *_ellipsoid* file"
        raise ValueError(msg)
    return name
