...
from insar_prep.desktop.helpers import _error, _error_msg, _normalise_proxy_url
...
def _clean_archive_scene_rows(value: Any) -> list[dict[str, Any]]:
    ...
    for raw in value[:10000]:
        if not isinstance(raw, dict):
            continue
        scene_id = str(raw.get("scene_id") or "").strip()
        if not scene_id:
            continue
        row = {key: raw.get(key) for key in allowed if key in raw}
        row["scene_id"] = scene_id[:260]
        rows.append(row)
    return rows
...
def _clean_download_archive(value: Any) -> list[dict[str, Any]]:
    ...
    for item in value[:80]:
        ...
        stable_task_id = str(item.get("task_id") or "").strip()
        ...
        snapshot = _clean_archive_scene_rows(
            item.get("snapshot") or item.get("scenes") or item.get("snapshot_scenes")
        )
        raw_scene_ids = item.get("scene_ids")
        scene_ids = (
            [
                str(scene_id).strip()[:260]
                for scene_id in raw_scene_ids[:10000]
                if str(scene_id).strip()
            ]
            if isinstance(raw_scene_ids, list)
            else []
        )
        if not scene_ids and snapshot:
            scene_ids = [str(row.get("scene_id") or "") for row in snapshot if row.get("scene_id")]
        aoi_name = str(item.get("aoi_name") or item.get("aoiName") or "").strip()
        try:
...
def _download_archive_identity(item: dict[str, Any]) -> str:
    ...
    task_id = str(item.get("id") or "").strip()
    stable_task_id = str(item.get("task_id") or "").strip()
    if kind and stable_task_id:
        return f"{kind}:task:{stable_task_id}"
...
def _download_archive_identity_candidates(
    ...
    *,
    ...
    if task_id:
        keys.add(task_id)
    if kind and stable_task_id:
        keys.add(f"{kind}:task:{stable_task_id}")
...
class Api:
    ...
    def __init__(self) -> None:
        ...
        self._v3_provider_service = V3ProviderDesktopService(self._state)
        self._gacos_webform_service = GacosWebFormService()
        ...
        self._download_status = DownloadStatusService(
            asf_status=lambda: self._asf_download.get_status(),
            orbit_status=lambda: self._orbit_download.get_status(),
            dem_status=lambda: self._dem_download.get_status(),
            archive_asf_status=self._archive_asf_status,
            archive_orbit_status=self._archive_orbit_status,
        )
        self._activity = ActivityLog()
        self._download_archive: list[dict[str, Any]] = []
        self._deleted_archive_keys: set[str] = set()
        self._download_archive_service = DownloadArchiveService(
            get_items=lambda: self._download_archive,
            set_items=lambda items: setattr(self, "_download_archive", items),
            get_deleted_keys=lambda: self._deleted_archive_keys,
            set_deleted_keys=lambda keys: setattr(self, "_deleted_archive_keys", keys),
            save_state=self._save_state,
            helpers={
                "clean_download_archive": _clean_download_archive,
                "filter_deleted_download_archive": _filter_deleted_download_archive,
                "dedupe_download_archive": _dedupe_download_archive,
                "merge_download_archive": _merge_download_archive,
                "download_archive_identity": _download_archive_identity,
                "download_archive_identity_candidates": _download_archive_identity_candidates,
                "normalise_download_archive_path": _normalise_download_archive_path,
                "clean_archive_scene_rows": _clean_archive_scene_rows,
                "format_status_log_entry": _format_status_log_entry,
                "coerce_bool": _coerce_bool,
            },
        )
        from insar_prep.desktop.aoi_preview_service import AoiPreviewDesktopService
        from insar_prep.desktop.credential_service import CredentialDesktopService

        self._credential_service = CredentialDesktopService(
            get_network_settings_cb=lambda: self._network_settings,
            default_cache_dir_cb=self._default_cache_dir,
            log_action_cb=self._act,
        )
        self._aoi_preview_service = AoiPreviewDesktopService(
            get_state_cb=lambda: self._state,
            save_state_cb=self._save_state,
            ensure_current_region_cb=self._ensure_current_region,
            log_action_cb=self._act,
        )
    ...
    def _load_state(self) -> None:
        ...
        try:
            ...
            if isinstance(flags, dict):
                self._ui_flags = {
                    str(key): bool(value) for key, value in flags.items() if isinstance(key, str)
                }
    ...
    def _asf_search_cache_bbox(bbox: Any) -> dict[str, Any] | None:
        ...
        try:
            return {
                "west": round(float(bbox.west), 7),
                "east": round(float(bbox.east), 7),
                "south": round(float(bbox.south), 7),
                "north": round(float(bbox.north), 7),
    ...
    def _asf_search_cache_aoi_hash(aoi_geojson: Any) -> str:
        ...
        raw = json.dumps(
            normalise(aoi_geojson), ensure_ascii=False, sort_keys=True, separators=(",", ":")
        )
    ...
    def _asf_cache_ranges_cover(cached: object, requested: object) -> bool:
        ...
        return all(
            any(
                cached_start <= req_start and cached_end >= req_end
    ...
    @staticmethod
    def _asf_cache_date_covers(
        cached_start: object, cached_end: object, requested_start: object, requested_end: object
    ) -> bool:
    ...
    def _asf_cache_query_covers(cached: dict[str, Any], requested: dict[str, Any]) -> bool:
        ...
        if cached.get("product_type") != requested.get("product_type"):
            return False
        if not Api._asf_cache_date_covers(
            cached.get("start"), cached.get("end"), requested.get("start"), requested.get("end")
        ):
            return False
        for name in ("beam_mode", "polarization", "orbit_direction"):
            if not Api._asf_cache_filter_covers(cached.get(name), requested.get(name)):
                return False
        return Api._asf_cache_ranges_cover(
            cached.get("relative_orbit"), requested.get("relative_orbit")
        ) and Api._asf_cache_ranges_cover(cached.get("frame"), requested.get("frame"))
    ...
    def _cached_asf_scenes_for_limit(
        ...
        if entry is None:
            return None
        scenes = Api._filter_cached_asf_scenes(
            list(entry.get("scenes") or []), requested_query or {}
        )
        if not scenes:
            return None
    ...
    @staticmethod
    def _cached_asf_partial_scenes(
        entry: dict[str, Any] | None,
        requested_query: dict[str, Any] | None = None,
    ) -> list[Any]:
        if entry is None:
            return []
        return Api._filter_cached_asf_scenes(list(entry.get("scenes") or []), requested_query or {})

    @staticmethod
    def _scene_acquisition_text(scene: Any) -> str:
        value = getattr(scene, "acquisition_datetime", None)
        return str(value or "").strip()

    @staticmethod
    def _incremental_asf_end_from_cache(scenes: list[Any]) -> str:
        dated = [
            Api._scene_acquisition_text(scene)
            for scene in scenes
            if Api._scene_acquisition_text(scene)
        ]
        if not dated:
            return ""
        oldest = min(dated)
        if "T" in oldest:
            return oldest
        return f"{oldest[:10]}T00:00:00Z" if len(oldest) >= 10 else ""
    ...
    def _filter_cached_asf_scenes(scenes: list[Any], query: dict[str, Any]) -> list[Any]:
        ...
        for scene in scenes:
            ...
            if orbit_values and text(getattr(scene, "orbit_direction", "")) not in orbit_values:
                continue
            if not in_ranges(
                getattr(scene, "relative_orbit", None) or getattr(scene, "path", None), path_ranges
            ):
                continue
    ...
    def save_network_settings(self, settings: dict | None = None) -> dict:
        ...
        tianditu_token = str(
            incoming.get("tianditu_token", current["tianditu_token"]) or ""
        ).strip()
        ...
        if tianditu_token and tianditu_token != previous_tianditu_token:
            ...
            if not results:
                err_detail = mask_text(error or "未返回行政边界数据")
                return _error_msg(
                    f"天地图 Key 校验失败，网络与缓存设置未保存：{err_detail}",
                    "GUI003",
                )
        ...
        self._apply_network_settings()
        self._credential_service._reset_earthdata_auth_cache()
        self._save_state()
    ...
    def get_download_archive(self) -> dict:
        ...
        return self._download_archive_service.get_archive()

    def _upsert_download_archive_item(self, item: dict[str, Any]) -> None:
        self._download_archive_service.upsert(item)
    ...
    def _forget_deleted_archive_key(
        ...
        *,
        ...
        self._download_archive_service.forget_deleted_key(
            kind,
            output_dir,
            use_product_subdirs=use_product_subdirs,
            use_orbit_subdir=use_orbit_subdir,
    ...
    def _archive_asf_status(self, status: dict[str, Any]) -> None:
        self._download_archive_service.archive_asf_status(status)

    def _archive_orbit_status(self, status: dict[str, Any]) -> None:
        self._download_archive_service.archive_orbit_status(status)

    def _archive_dem_status(self, status: dict[str, Any]) -> None:
        self._download_archive_service.archive_dem_status(status)

    def save_download_archive(self, items: list[dict[str, Any]] | None = None) -> dict:
        """Persist the UI download/task history."""
        return self._download_archive_service.save_archive(items)
    ...
    def delete_download_archive_item(self, item: dict[str, Any] | None = None) -> dict:
        ...
        return self._download_archive_service.delete_archive_item(item)
    ...
    def check_for_update(self, force: bool = False) -> dict:
        ...
        try:
            ...
            info = (
                ...
                if bool(force)
                else maybe_check_for_update(force=True, interval_seconds=60 * 60, timeout=10)
            )
    ...
    def download_app_update(
        ...
        mirrors = [
            str(item or "").strip() for item in (mirror_urls or []) if str(item or "").strip()
        ]
        try:
        ...
        try:
            ...
            if is_installer:
                ...
                launched = True
                message = (
                    "安装器已下载并启动。若当前窗口没有自动关闭，请手动关闭后等待覆盖安装完成。"
                )
    ...
    def get_workflow_status(self) -> dict:
        ...
        return {
            ...
            "scene_coverage": {
                ...
                "count": sum(
                    1 for s in (region.scenes if region is not None else []) if s.footprint_bbox
                ),
            },
            "dem_dataset": self._dem_dataset,
        }
    ...
    # --------------------------------------------------------------- v3 CORE
    def get_v3_providers(self) -> dict:
        """Return providers exposed through the v3 adapter registry."""
        try:
            providers = self._v3_provider_service.provider_metadata()
        except Exception as exc:  # noqa: BLE001
            return _error_msg(str(exc), "GUI003")
        return {"ok": True, "providers": [_dump(item) for item in providers]}

    def v3_search_provider(self, provider_id: str, query: dict | None = None) -> dict:
        """Search a v3 provider through the compatibility adapter layer."""
        try:
            products = self._v3_provider_service.search_provider(provider_id, query)
        except InsarPrepError as exc:
            return _error(exc)
        except Exception as exc:  # noqa: BLE001
            return _error_msg(str(exc), "GUI003")
        return {"ok": True, "products": [_dump(item) for item in products]}

    def v3_plan_provider(
        self,
        provider_id: str,
        query: dict | None = None,
        products: list[dict] | None = None,
    ) -> dict:
        """Build an offline v3 provider plan without executing downloads."""
        try:
            plan = self._v3_provider_service.plan_provider(provider_id, query, products)
        except InsarPrepError as exc:
            return _error(exc)
        except Exception as exc:  # noqa: BLE001
            return _error_msg(str(exc), "GUI003")
        self._act(f"v3 provider plan: {provider_id}", kind="download")
        return {"ok": True, "plan": _dump(plan)}

    def gacos_webform_dry_run(self, payload: dict, email: str = "") -> dict:
        """Dry-run format GACOS web form fields and generate curl previews."""
        try:
            return self._gacos_webform_service.dry_run_preview(payload, email)
        except InsarPrepError as exc:
            return _error(exc)
        except Exception as exc:  # noqa: BLE001
            return _error_msg(str(exc), "GUI003")

    def gacos_webform_submit(self, payload: dict, email: str) -> dict:
        """Reserved for future GACOS web form real POST submissions."""
        try:
            return self._gacos_webform_service.submit_webform(payload, email)
        except InsarPrepError as exc:
            return _error(exc)
        except Exception as exc:  # noqa: BLE001
            return _error_msg(str(exc), "GUI003")
        ...
        try:
            ...
            if self._window_maximized:
                ...
                if bounds is not None:
                    ...
                    window.resize(
                        int(bounds["width"]), int(bounds["height"]), FixPoint.NORTH | FixPoint.WEST
                    )
                else:
                    window.restore()
                self._window_maximized = False
                ...
                target_area = (
                    self._normalise_window_work_area(work_area) or self._windows_work_area()
                )
    ...
    def shutdown(self) -> dict:
        ...
        try:
            ...
            self._archive_asf_status(
                asf_before
                if asf_before.get("state") == "paused"
                else self._asf_download.get_status()
            )
        except Exception as exc:  # noqa: BLE001
        ...
        try:
            ...
            self._archive_orbit_status(
                orbit_before
                if orbit_before.get("state") == "paused"
                else self._orbit_download.get_status()
            )
        except Exception as exc:  # noqa: BLE001
    ...
    def set_region_aoi_file(self, path: str) -> dict:
        return self._aoi_preview_service.set_region_aoi_file(path)

    def preview_aoi_file(self, path: str) -> dict:
        return self._aoi_preview_service.preview_aoi_file(path)

    def preview_aoi_file_content(self, file_name: str, text: str) -> dict:
        return self._aoi_preview_service.preview_aoi_file_content(file_name, text)

    def preview_aoi_file_bytes(self, file_name: str, base64_data: str) -> dict:
        return self._aoi_preview_service.preview_aoi_file_bytes(file_name, base64_data)

    def preview_aoi_file_bundle(self, files: list[dict[str, Any]] | None = None) -> dict:
        return self._aoi_preview_service.preview_aoi_file_bundle(files)
    ...
    def set_region_aoi_file_features(
        ...
        return self._aoi_preview_service.set_region_aoi_file_features(
            path, feature_ids, name_field, download_mode
        )

    def set_region_aoi_geojson(self, geojson: dict) -> dict:
        return self._aoi_preview_service.set_region_aoi_geojson(geojson)
    ...
    def set_region_aoi_geojson_features(
        ...
        return self._aoi_preview_service.set_region_aoi_geojson_features(
            geojson, feature_ids, name_field, download_mode
        )

    def clear_region_aoi(self) -> dict:
        return self._aoi_preview_service.clear_region_aoi()
    ...
    def search_admin_boundaries(
        ...
        if _is_national_admin_request(
            ...
            results, datav_error = _search_datav_admin_boundaries(
                province=str(province or ""),
                city=str(city or ""),
                district=str(district or ""),
                query=str(query or ""),
            ...
            self._act("行政边界搜索：全国", kind="aoi")
            return {
                "ok": True,
                "query": "全国",
                "provider": "datav"
                if results and results[0].get("source") == "DataV 行政区划服务"
                else "builtin",
                "results": results or [_builtin_china_boundary()],
                "warning": f"DataV 全国边界不可用，已使用内置全国范围：{datav_error}"
                if datav_error
                else None,
            }
        ...
        province_value = _admin_value(province)
        taiwan_division = _admin_value(district) or _admin_value(city)
        if province_value == "台湾省" and taiwan_division:
            taiwan_results, taiwan_error = _search_nominatim_taiwan_boundary(
                taiwan_division, limit=limit
            )
            if taiwan_results:
                self._act(f"台湾行政边界搜索：{taiwan_division}", kind="aoi")
                return {
                    "ok": True,
                    "query": f"台湾省 {taiwan_division}",
                    "provider": "nominatim",
                    "results": taiwan_results,
                    "warning": None,
                }
            return _error_msg(
                f"未能取得 {taiwan_division} 的真实行政边界：{taiwan_error or '开放边界源未返回面数据'}。"
                "请稍后重试，或上传权威 shp/kml/geojson 边界。",
                "AOI001",
            )

        source_warnings: list[str] = []
        datav_results, datav_error = _search_datav_admin_boundaries(
            province=str(province or ""),
            city=str(city or ""),
            district=str(district or ""),
            query=str(query or ""),
        ...
        if datav_results:
            self._act(f"DataV 行政边界搜索：{search_text}（{len(datav_results)} 条）", kind="aoi")
            return {
                "ok": True,
                "query": search_text,
                "provider": "datav",
                "results": datav_results,
                "warning": None,
            }
        if datav_error:
            source_warnings.append(datav_error)
        ...
        if local_results:
            ...
            return {
                ...
                "warning": "DataV 行政区服务不可用，已回退到本地边界。"
                if source_warnings
                else None,
            }
        if local_error:
        ...
        if token:
            ...
            if tianditu_results:
                self._act(
                    f"天地图行政边界搜索：{search_text}（{len(tianditu_results)} 条）", kind="aoi"
                )
                return {
                    "ok": True,
                    "query": search_text,
    ...
    def get_admin_options(self, province: str = "", city: str = "") -> dict:
        """Return province/city/county option lists from DataV, with local fallback."""
        try:
            options = _datav_admin_options(province=province, city=city)
            # DataV 对台湾省子级返回空列表；通过 Nominatim 查询县市，本地 GeoJSON 兜底。
            province_val = _admin_value(province)
            if province_val == "台湾省":
                taiwan_names = _nominatim_taiwan_admin_options()
                options["cities"] = ["全部", *taiwan_names]
                options["districts"] = ["全部"]
        ...
        try:
            ...
            return _error_msg(
                f"行政区划选项读取失败：DataV {datav_error}；本地边界 {exc}", "AOI001"
            )
        return {
            "ok": True,
            **options,
            "warning": f"DataV 行政区服务不可用，已回退到本地边界：{datav_error}",
        }
    ...
    def import_scenes_text(self, text: str) -> dict:
        ...
        unique = _enrich_scenes_best_effort(
            ...
            progress=lambda done, total, msg: self._set_metadata_status(
    ...
    def import_scenes_directory(self, path: str) -> dict:
        ...
        for entry in sorted(root.rglob("*")):
            ...
            if not (
                (entry.is_file() and suffix == ".zip") or (entry.is_dir() and suffix == ".safe")
            ):
                continue
            try:
    ...
    def clear_orbit_candidate_scenes(self) -> dict:
        ...
        self._act(
            f"已清除 {cleared} 景精密轨道预览候选，恢复沿用 Sentinel-1 检索结果", kind="orbit"
        )
    ...
    def clear_map_layers(self) -> dict:
        ...
        msg = (
            f"已清空 {label} 的地图图层：AOI {1 if cleared_aoi else 0} 个，"
            f"影像 {cleared_scenes} 景"
        )
        self._act(
            msg,
            kind="aoi",
    ...
    def search_asf_scenes(self, params: dict | None = None) -> dict:
        ...
        bbox = None
        aoi_geojson = (
            payload.get("aoi_geojson") if isinstance(payload.get("aoi_geojson"), dict) else None
        )
        ...
        try:
            ...
            frame = _optional_asf_int_filter(
                "frame", start_name="frame_start", end_name="frame_end"
            )
            ...
            cached_scenes = self._cached_asf_scenes_for_limit(
                cache_entry, explicit_limit, cache_query
            )
            if cached_scenes is None:
                covering_entry = self._find_covering_asf_search_cache(
                    cache_query, explicit_limit, cache_path
                )
            ...
            if cached_scenes is not None:
                ...
                self._set_metadata_status(
                    "finished", 1, 1, f"已从本地缓存读取 ASF 检索结果：{len(stored)} 景"
                )
                ...
                return {
                    ...
                    "search": {
                        ...
                        "query_limit": len(cache_entry.get("scenes") or [])
                        if cache_entry
                        else len(scenes),
                        "total_count": cached_total,
                        "candidate_count": cached_total
                    ...
                    "cache": {
                        ...
                        "count": len(cache_entry.get("scenes") or [])
                        if cache_entry
                        else len(scenes),
                        "path": cache_entry.get("path") if cache_entry else "",
                    },
                }
            partial_cached_scenes = self._cached_asf_partial_scenes(cache_entry, cache_query)
            incremental_end = self._incremental_asf_end_from_cache(partial_cached_scenes)
            incremental_missing = (
                max(0, int(explicit_limit or 0) - len(partial_cached_scenes))
                if explicit_limit and partial_cached_scenes and incremental_end
                else 0
            )
            if incremental_missing > 0:
                payload_end = str(payload.get("end") or "").strip() or None
                search_end = incremental_end
                if payload_end and payload_end < incremental_end:
                    search_end = payload_end
                self._set_metadata_status(
                    "running",
                    len(partial_cached_scenes),
                    explicit_limit,
                    f"已有缓存 {len(partial_cached_scenes)} 景，补查 {incremental_missing} 景",
                )
                extra_stats: dict[str, Any] = {}
                try:
                    extra_scenes = search_scenes_from_asf(
                        bbox=bbox,
                        aoi_geojson=aoi_geojson,
                        ...
                        relative_orbit=relative_orbit,
                        frame=frame,
                        max_results=incremental_missing,
                        progress=lambda done, total, msg: self._set_metadata_status(
                            "running", done, total, msg
                        ),
                        stats=extra_stats,
                    ...
                    scenes, _ = deduplicate_scenes([*partial_cached_scenes, *extra_scenes])
                    scenes = _sort_scenes_by_acquisition_date(scenes)[:explicit_limit]
                    search_stats.update(
                        {
                            "requested_limit": explicit_limit,
                            "query_limit": len(partial_cached_scenes)
                            + int(extra_stats.get("query_limit") or incremental_missing),
                            "total_count": extra_stats.get("total_count")
                            or (cache_entry or {}).get("total_count"),
                            "candidate_count": extra_stats.get("candidate_count")
                            or (cache_entry or {}).get("total_count"),
                            "returned_count": len(scenes),
                            "source": "ASF_INCREMENTAL",
                        }
                    )
                    self._write_asf_search_cache(
                        ...
                        scenes=scenes,
                        stats={**search_stats, "source": "ASF"},
                        existing=cache_entry,
                    )
                    stored, label = self._store_imported_scenes(scenes)
                    self._set_metadata_status(
                        "finished", 1, 1, f"ASF 补量完成：导入 {len(stored)} 景"
                    )
                    self._act(f"ASF 补量导入 {len(stored)} 景 → {label}", kind="scenes")
                    return {
                        "ok": True,
                        "scenes": [_scene_row(s) for s in stored],
                        "duplicates": [],
                        ...
                        "search": {
                            ...
                            "candidate_count": search_stats.get("candidate_count"),
                            "returned_count": len(stored),
                            "source": "ASF_INCREMENTAL",
                        },
                        "cache": {
                            ...
                            if cache_entry
                            else str(cache_path or ""),
                        },
                    }
                except InsarPrepError:
                    raise
                except Exception as exc:  # noqa: BLE001
                    self._activity.add(f"ASF 缓存补量失败，改为完整检索：{exc}", kind="warning")
            ...
            stored, label = self._store_imported_scenes(scenes)
            candidate_count = (
                search_stats.get("candidate_count")
                or search_stats.get("total_count")
                or len(scenes)
            )
            target_count = search_stats.get("requested_limit") or explicit_limit or len(stored)
            status_text = (
                f"ASF 检索完成：目标 {target_count} 景，"
                f"候选 {candidate_count} 景，导入 {len(stored)} 景"
            )
            self._set_metadata_status(
                "finished",
                1,
            ...
            if getattr(exc, "code", None) and str(exc.code) == "DL001":
                self._set_metadata_status("cancelled", 0, 1, "ASF 检索已停止")
                return {"ok": False, "cancelled": True, "error": "ASF 检索已停止", "code": "DL001"}
    ...
    def download_orbits(
        ...
        self._last_orbit_report = report
        act_msg = (
            f"精密轨道下载：{label}，{summary.summary_line()}，"
            f"匹配 {report.matched_scenes}/{report.total_scenes} 景"
        )
        self._act(
            act_msg,
            kind="download",
    ...
    def start_orbit_download_snapshot(
        ...
        try:
            scenes = [
                _scene_from_row(row) for row in (scenes_snapshot or []) if isinstance(row, dict)
            ]
        except Exception as exc:  # noqa: BLE001
        ...
        if selected_ids:
            scenes = [
                scene for scene in scenes if str(getattr(scene, "scene_id", "")) in selected_ids
            ]
        return self._start_orbit_download_for_scenes(
            scenes,
    ...
    def _start_orbit_download_for_scenes(
        ...
        *,
        ...
        Path(out) / "Sentinel_Orbit" / "AUX_POEORB" if use_subdir else Path(out)
        self._forget_deleted_archive_key("orbit", out, use_orbit_subdir=use_subdir)
        result = self._orbit_download.start(
            scenes,
            out,
            max_concurrent=max_concurrent,
            use_orbit_subdir=use_subdir,
            aoi_name=aoi_name or label,
            activity=self._activity,
        )
    ...
    def get_orbit_download_status(self) -> dict:
        ...
        return self._download_status.get_orbit_status()
    ...
    def _start_asf_download_for_scenes(
        ...
        scenes: list[Any],
        *,
        snapshot_scenes: list[Any] | None = None,
        label: str,
        aoi_name: str = "",
        output_dir: str = "",
        credential_source: str = "auto",
        ...
        try:
            ...
            if not requests:
                return _error_msg(
                    "当前影像没有 ASF 下载 URL，请先重新检索或导入 ASF 官方文件。", "ASF003"
                )
        ...
        result = self._asf_download.start(
            ...
            use_product_subdirs=use_subdirs,
            aoi_name=aoi_name or label,
            snapshot_scenes=snapshot_scenes or scenes,
            activity=self._activity,
        )
    ...
    def pause_asf_scenes(self, scene_ids: list[str] | None = None, task_id: str = "") -> dict:
        ...
        result = self._asf_download.pause_scenes(
            scene_ids or [], task_id=str(task_id or "").strip()
        )
        self._archive_asf_status(self._asf_download.get_status())
        return result
    ...
    def resume_asf_scenes(self, scene_ids: list[str] | None = None, task_id: str = "") -> dict:
        ...
        result = self._asf_download.resume_scenes(
            scene_ids or [], task_id=str(task_id or "").strip()
        )
        self._archive_asf_status(self._asf_download.get_status())
        return result
    ...
    def retry_asf_download(self, task_id: str = "") -> dict:
        ...
        result = self._asf_download.retry_failed(
            activity=self._activity, task_id=str(task_id or "").strip()
        )
        self._archive_asf_status(self._asf_download.get_status())
        return result
    ...
    def get_download_status(self) -> dict:
        ...
        return self._download_status.get_asf_status()
    ...
    def pause_dem_download(self) -> dict:
        """Pause the active DEM transfer without discarding its partial file."""
        return self._dem_download.pause()

    def resume_dem_download(self) -> dict:
        """Resume a paused DEM transfer."""
        return self._dem_download.resume()

    def get_dem_download_status(self) -> dict:
        """Return current background DEM download status."""
        return self._download_status.get_dem_status()
    ...
    def check_earthdata_auth(self, force: bool = False) -> dict:
        return self._credential_service.check_earthdata_auth(force)

    def save_earthdata_token(self, token: str) -> dict:
        return self._credential_service.save_earthdata_token(token)

    def save_earthdata_login(self, username: str, password: str) -> dict:
        return self._credential_service.save_earthdata_login(username, password)

    def clear_earthdata_credentials(self) -> dict:
        return self._credential_service.clear_earthdata_credentials()

    def save_opentopography_key(self, api_key: str) -> dict:
        return self._credential_service.save_opentopography_key(api_key)

    def clear_opentopography_key(self) -> dict:
        return self._credential_service.clear_opentopography_key()

    def save_gacos_email(self, email: str) -> dict:
        return self._credential_service.save_gacos_email(email)

    def clear_gacos_email(self) -> dict:
        return self._credential_service.clear_gacos_email()

    def get_credential_status(self) -> dict:
        return self._credential_service.get_credential_status()
    ...
    def _run_dem_download_plan(self, plan: Any, output_dir: str, key_source: str = "auto") -> dict:
    ...
    def _run_dem_download_and_conversion_plan(
        ...
        if conversion is None:
            ...
            summary_line = (
                f"下载：{download.get('summary_line')}；转换：{conversion.get('summary_line')}"
            )
            logs.extend(str(item) for item in conversion.get("logs", []) if item)
        else:
            summary_line = (
                f"下载：{download.get('summary_line')}；转换失败：{conversion.get('error')}"
            )
        ...
        return {
            **download,
            ...
            "raw_dem_path": str(getattr(plan, "raw_dem_path", "")),
            "ellipsoid_dem_path": str(getattr(conversion_plan or plan, "ellipsoid_dem_path", "")),
    ...
    def _run_dem_conversion_plan(
        ...
        return {
            ...
            "sarscape_ready_dem_path": (
                ""
                if output_mode == "ellipsoid"
                else str(getattr(conv_plan, "sarscape_ready_dem_path", ""))
            ),
        }
...
def _conversion_summary(conv_plan: Any) -> dict:
    ...
    if not requires:
        message = (
            f"该 DEM 高程基准已为 {source}（椭球高），无需垂直基准转换，"
            f"将导出 SARscape ENVI _dem + .hdr + .sml。"
        )
...
_DATAV_CHILDREN_CACHE: dict[str, list[dict[str, str]]] = {}
_DATAV_BOUNDARY_CACHE: dict[str, dict] = {}
_NOMINATIM_TAIWAN_CACHE: list[str] = []
_TAIWAN_DIVISIONS = (
...
_DATAV_BOUNDARY_ROOT = "https://geo.datav.aliyun.com/areas_v3/bound"
_DATAV_PROVINCES: tuple[tuple[str, str], ...] = (
    ("110000", "北京市"),
    ("120000", "天津市"),
    ("130000", "河北省"),
    ("140000", "山西省"),
...
def _datav_cache_dir() -> Path:
    env_path = os.environ.get("INSAR_ADMIN_CACHE_DIR")
    if env_path:
        return Path(env_path)
    local_appdata = os.environ.get("LOCALAPPDATA")
    if local_appdata:
        return Path(local_appdata) / "InSAR Assistant" / "admin_cache"
    return Path.home() / ".insar_assistant" / "admin_cache"


def _datav_cache_file(kind: str, code: str) -> Path:
    safe_code = re.sub(r"[^0-9A-Za-z_-]+", "_", str(code or "").strip() or "empty")
    return _datav_cache_dir() / f"datav_{kind}_{safe_code}.json"


def _read_datav_json_cache(kind: str, code: str) -> dict | None:
    path = _datav_cache_file(kind, code)
    if not path.is_file():
        return None
    try:
        data = json.loads(path.read_text(encoding="utf-8"))
    except Exception:  # noqa: BLE001 - stale cache is non-fatal
        return None
    return data if isinstance(data, dict) else None


def _write_datav_json_cache(kind: str, code: str, data: dict) -> None:
    try:
        path = _datav_cache_file(kind, code)
        path.parent.mkdir(parents=True, exist_ok=True)
        path.write_text(json.dumps(data, ensure_ascii=False), encoding="utf-8")
    except Exception:  # noqa: BLE001 - cache write must not break AOI search
        return


def _fetch_datav_json(kind: str, code: str) -> dict:
    suffix = "_full" if kind == "children" else ""
    url = f"{_DATAV_BOUNDARY_ROOT}/{code}{suffix}.json"
    try:
        import requests  # noqa: PLC0415 - desktop optional network extra

        response = requests.get(
            url,
            headers={"User-Agent": "InSAR-Studio/2 DataV administrative boundary"},
            timeout=20,
        )
        response.raise_for_status()
        data = response.json()
        if not isinstance(data, dict):
            raise ValueError("DataV 返回的行政区数据不是 JSON 对象")
        _write_datav_json_cache(kind, code, data)
        return data
    except Exception as exc:  # noqa: BLE001
        cached = _read_datav_json_cache(kind, code)
        if cached is not None:
            return cached
        raise RuntimeError(f"DataV 行政区服务不可用，且本机没有缓存：{exc}") from exc


def _datav_children(code: str) -> list[dict[str, str]]:
    clean_code = str(code or "").strip()
    if not clean_code:
        return []
    cached = _DATAV_CHILDREN_CACHE.get(clean_code)
    if cached is not None:
        return cached
    try:
        data = _fetch_datav_json("children", clean_code)
    except Exception as exc:  # noqa: BLE001
        # DataV has no *_full.json for leaf districts/counties. Treat that as
        # "no children" instead of falling back to mismatched local options.
        text = str(exc)
        if "404" in text or "Not Found" in text:
            _DATAV_CHILDREN_CACHE[clean_code] = []
            return []
        raise
    ...
    for feature in data.get("features") or []:
        ...
        if raw_code is None or not name:
            continue
        rows.append({"code": str(raw_code), "name": name})
    _DATAV_CHILDREN_CACHE[clean_code] = rows
    return rows


def _datav_province_rows() -> list[dict[str, str]]:
    return [{"code": code, "name": name} for code, name in _DATAV_PROVINCES]


def _datav_name_matches(name: str, query: str) -> bool:
    if not query:
        return True
    return name == query or query in name or name in query


def _datav_resolve_row(value: str, rows: list[dict[str, str]]) -> dict[str, str] | None:
    text = _admin_value(value)
    if not text:
        return None
    exact = [row for row in rows if row["code"] == text or row["name"] == text]
    if exact:
        return exact[0]
    fuzzy = [row for row in rows if _datav_name_matches(row["name"], text)]
    return fuzzy[0] if fuzzy else None


def _datav_admin_options(*, province: str = "", city: str = "") -> dict[str, list[str]]:
    province_rows = _datav_province_rows()
    province_row = _datav_resolve_row(province, province_rows)
    city_rows = _datav_children(province_row["code"]) if province_row else []
    city_row = _datav_resolve_row(city, city_rows) if city_rows else None
    district_rows = _datav_children(city_row["code"]) if city_row else []
    return {
        "provinces": ["全国", *[row["name"] for row in province_rows]],
        "cities": ["全部", *[row["name"] for row in city_rows]],
        "districts": ["全部", *[row["name"] for row in district_rows]],
    }
...
def _gcj02_to_wgs84(lng: float, lat: float) -> tuple[float, float]:
    ...
    x = lng - 105.0
    y = lat - 35.0
    dlat = _transform_lat(x, y)
    dlng = _transform_lng(x, y)
...
def _transform_datav_coordinates(value: Any) -> None:
    if isinstance(value, dict):
        coords = value.get("coordinates")
        if isinstance(coords, list):
            _transform_datav_coordinates(coords)
        for key, child in value.items():
            if key != "coordinates":
                _transform_datav_coordinates(child)
        return
    if isinstance(value, list):
        if (
            len(value) >= 2
            ...
            lng, lat = _gcj02_to_wgs84(float(value[0]), float(value[1]))
            value[0] = lng
            value[1] = lat
            return
        for child in value:
...
def _walk_datav_positions(value: Any, out: list[tuple[float, float]]) -> None:
    if isinstance(value, dict):
        if "coordinates" in value:
            _walk_datav_positions(value.get("coordinates"), out)
        for key in ("geometry", "geometries", "features"):
            _walk_datav_positions(value.get(key), out)
        return
    if isinstance(value, list):
        if (
            len(value) >= 2
            ...
            return
        for child in value:
            _walk_datav_positions(child, out)


def _datav_geojson_bbox(geojson: dict) -> dict | None:
    positions: list[tuple[float, float]] = []
    _walk_datav_positions(geojson, positions)
    if not positions:
        return None
...
def _datav_boundary(code: str) -> dict:
    clean_code = str(code or "").strip()
    if not clean_code:
        raise ValueError("缺少行政区代码")
    cached = _DATAV_BOUNDARY_CACHE.get(clean_code)
    if cached is not None:
        return cached
    geojson = _fetch_datav_json("boundary", clean_code)
    _transform_datav_coordinates(geojson)
    _DATAV_BOUNDARY_CACHE[clean_code] = geojson
    return geojson


def _datav_boundary_result(code: str, label: str, *, admin_type: str | None = None) -> dict:
    try:
        geojson = _datav_boundary(code)
    except Exception:
        raise
    bbox = _datav_geojson_bbox(geojson) if isinstance(geojson, dict) else None
    if bbox is None:
        raise ValueError(f"DataV 行政边界缺少有效坐标：{label}")
    return {
        "label": label,
        "bbox": bbox,
        "geojson": geojson,
...
def _search_datav_admin_boundaries(
    *,
    province: str,
    city: str,
    district: str,
    ...
    if _is_national_admin_request(province=province, city=city, district=district, query=query):
        try:
            return [_datav_boundary_result("100000", "全国", admin_type="country")], None
        except Exception as exc:  # noqa: BLE001
            return [_builtin_china_boundary()], str(exc)
    ...
    city_row = _datav_resolve_row(city, city_rows) if city_rows else None
    district_rows = _datav_children(city_row["code"]) if city_row else []
    district_row = _datav_resolve_row(district, district_rows) if district_rows else None
    query_text = _admin_value(query)
    results: list[dict] = []

    def add(row: dict[str, str], parts: list[str], admin_type: str) -> bool:
        if len(results) >= max_items:
            return True
        label = " / ".join(dict.fromkeys([*parts, row["name"]]))
        try:
            results.append(_datav_boundary_result(row["code"], label, admin_type=admin_type))
        except Exception:
            return False
    ...
    if district_row:
        add(
            district_row,
            [province_row["name"] if province_row else "", city_row["name"] if city_row else ""],
            "district",
        )
        return results, None
    if city_row:
        add(city_row, [province_row["name"] if province_row else ""], "city")
        return results, None
    if province_row:
        add(province_row, [], "province")
        return results, None
    if query_text:
        for row in province_rows:
            if _datav_name_matches(row["name"], query_text) and add(row, [], "province"):
                return results, None
        for prow in province_rows:
            try:
                for crow in _datav_children(prow["code"]):
                    if _datav_name_matches(crow["name"], query_text) and add(
                        crow, [prow["name"]], "city"
                    ):
                        return results, None
            except Exception:
...
def _load_local_boundary_layer(level: str) -> list[dict]:
    ...
    clean = [
        ...
        if isinstance(feature, dict) and _local_feature_name(feature)
    ]
...
def _is_national_admin_request(*, province: str, city: str, district: str, query: str) -> bool:
    ...
    return (
        not city_text
        and not district_text
        and (province_text in {"全部", "全国", "中国"} or query_text in {"全部", "全国", "中国"})
    )
...
def _builtin_china_boundary() -> dict:
    ...
    if geojson is None:
        raise ValueError("缺少可用的本地全国真实行政边界")
    return {
        "label": "全国",
        "bbox": {
            "west": 73.5,
...
def _append_unique_boundary(
    results: list[dict], feature: dict, label_parts: list[str], limit: int
) -> None:
...
def _local_admin_options(*, province: str = "", city: str = "") -> dict[str, list[str]]:
    ...
    province_names = sorted(
        {_local_feature_name(feature) for feature in provinces if _local_feature_name(feature)}
    )
    province_name = _admin_value(province)
    city_name = _admin_value(city)
    province_feature = _find_feature(provinces, province_name) if province_name else None
    ...
    city_features = [
        ...
        and (not province_prefix or _local_feature_gb(feature).startswith(province_prefix))
    ]
    ...
    county_features = [
        ...
        and (
            ...
            or (
                not city_prefix
                and province_prefix
                and _local_feature_gb(feature).startswith(province_prefix)
            )
        )
    ]
    county_names = sorted({_local_feature_name(feature) for feature in county_features})
    if len(city_names) == 1 and city_names[0] == province_name and not county_names:
        city_names = []
    return {
        "provinces": ["全国", *province_names],
        "cities": ["全部", *city_names],
        "districts": ["全部", *county_names],
    }


def _nominatim_taiwan_admin_options() -> list[str]:
    """Return Taiwan first-level divisions for the city selector."""
    if _NOMINATIM_TAIWAN_CACHE:
        return list(_NOMINATIM_TAIWAN_CACHE)
    _NOMINATIM_TAIWAN_CACHE.extend(_TAIWAN_DIVISIONS)
    return list(_NOMINATIM_TAIWAN_CACHE)


def _search_nominatim_taiwan_boundary(
    name: str, *, limit: int = 8
) -> tuple[list[dict], str | None]:
    """Resolve one Taiwan division to a real OSM polygon boundary."""
    division = _admin_value(name)
    if not division:
        return [], "未选择县市"
    try:
        ...
        response = requests.get(
            ...
            params={
                ...
                "polygon_geojson": "1",
                "addressdetails": "1",
                "limit": str(max(1, min(int(limit or 8), 10))),
                "accept-language": "zh-CN,zh,en",
            },
            headers={"User-Agent": "InSAR-Studio/2 Taiwan administrative boundary"},
            timeout=15,
        )
        response.raise_for_status()
        data = response.json()
    ...
    for feature in features:
        ...
        break
    return results, None if results else "未返回 Polygon/MultiPolygon"
...
def _tianditu_admin_items(value: Any) -> list[dict]:
    ...
    def walk(node: Any) -> None:
        if isinstance(node, dict):
            has_name = any(
                k in node for k in ("name", "gb", "cityCode", "english", "nameabbrevation")
            )
            has_geometry = any(
                k in node for k in ("bound", "points", "region", "coordinates", "geometry")
            )
